...
class CombinedCVAE(nn.Module):
    def __init__(self, feature_dim, num_classes, latent_dim=128, hidden_dims=[512, 256], dropout_prob=0.1):
        """
        feature_dim: 输入特征维度（例如 512）
        num_classes: 类别数（用于 one-hot 编码）
        latent_dim: 隐变量维度（默认128）
        hidden_dims: 编码器/解码器中间隐层尺寸（默认[512,256]）
        ...
        input_dim = feature_dim + num_classes  # 拼接特征与类别 one-hot

        # 编码器: 两层全连接，使用 BatchNorm 和 Dropout 提升泛化
    ...
    def encode(self, x, labels):
        ...
        labels: [batch_size, num_classes]，通常为 one-hot 向量
        """
    ...
    def forward(self, x, labels):
        """
        x: [B, feature_dim]
        labels: [B, num_classes] one-hot 表示
        """
...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.fc = None  # 用于分类的全连接层，后面 update_fc 会设置
...
class CGRIncrementalNet(BaseNet):
    ...
    def set_gradcam_hook(self):
        ...
        # 假设你的resnet最后一层卷积是 self.convnet.last_conv
