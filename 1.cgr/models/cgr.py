...
from utils.inc_net import CGRNet
...
# ============ CGR 主类: 引入 CVAE 回放 ============ #
####################################################
class CGR(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._network = CGRNet(args, False)
        ...
        # encoder_k 用于对比损失
        self.encoder_k = CGRNet(args, False).convnet
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task in range(self.args["ckpt_num"]):
            ...
            detail = p.split('/')
            l = "{}_{}_{}_{}.pkl".format('cgr', detail[-3], detail[-2], self._cur_task)
            l = os.path.join(p, l)
    ...
    def _compute_il2a_loss(self, inputs, targets, image_k=None):
        """
        GFR-CGR: 在原CGR基础上整合 CVAE (旧特征回放) + 对比学习 
        """
        # 先做一次前向
