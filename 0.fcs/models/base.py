...
class BaseLearner(object):
    ...
    def eval_task(self,only_old=False,only_new=False):
    ...
    def incremental_train(self,args):
        pass
    ...
    def _extract_old_vectors(self, loader):
        ...
        for _, _inputs, _targets in loader:
            ...
            if isinstance(self._old_network, nn.DataParallel):
                _vectors = tensor2numpy(
                    self._old_network.module.extract_vector(_inputs.to(self._device))
                )
            else:
                _vectors = tensor2numpy(
                    self._old_network.extract_vector(_inputs.to(self._device))
                )
